...
# 50190wxapp在线厨艺平台的设计与实现微信小程序ssm
...
图4-6 食材分类实体属性图
### 4.3.2 数据库表结构
在进行这部分设计之前，需要明白和掌握数据类型以及各个数据类型的长度范围等知识，因为在一张具体的数据表中，为了方便理解，这里就举个简单的例子。比如用户信息表，这个表格的字段就是用户这个实体具备的属性，这时就需要对字段进行数据类型，以及字段长度的设置，也要设置一个主键来作为用户信息表的唯一标识。这些都是数据库表结构设计需要完成的内容。根据在线厨艺平台的设计与实现微信小程序的功能设计以及数据库设计要求，展示该系统的数据表结构。 
...
|4|kouwei|String|口味|是|
|5|caipufenlei|String|菜谱分类|是|
|6|shiherenqun|String|适合人群|是|
|7|tupian|String|图片|是|
|8|shicai|String|食材|是|
|9|yingyang|String|营养|是|
|10|zhizuobuzhou|String|制作步骤|是|
...
|1|Id|Int|id|否|
|2|addtime|Date|创建时间|是|
|3|shicaimingcheng|String|食材名称|是|
|4|shicaifenlei|String|食材分类|是|
|5|shicaitupian|String|食材图片|是|
|6|shicaigongxiao|String|食材功效|是|
|7|shicaijieshao|String|食材介绍|是|
...
图5-3 食材分类页面
### 5.1.4 菜谱信息
此页面显示菜谱信息信息。菜谱信息页面见下图。管理员可以在此界面对菜谱信息进行添加，修改，查询，删除等操作。
